<template>
    <div class="container shadow-lg px-5 py-5 rounded-3">
        <div class="task">
          <h1>All Task</h1>
          <div v-for="(item, index) in TODO.task.newTodo" :key="index" class="form-control p-3 mb-2 ">
            <input
              type="checkbox"
              :name="item.id"
              id=""
            ...
            />
            <h4 class="d-inline-flex bg-warning text-dark mx-2  " >{{ item.category.join(", ") }}</h4>
            <button @click="TODO.deleteTask(item.id)" class="btn btn-danger">Delete</button>
          </div>
        </div>
    </div>
  </template>
